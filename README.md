...
За основу взят агент [https://github.com/Lucretius/vault_raft_snapshot_agent], отличия:
1. Убраны лишние провайдеры, вроде GCP, Azure и т.п.
2. Добавлен custom http client для обхода невалидного сертификака в hitachi
3. Добавлено шифрование итового архива с использованием pgp.

Оригинальное описание ниже, с добавлением нововедеений.
...
`skip_ssl_verify` Проверка SSL сертификата в AWS, true игнорирует проверку и работает с самоподписанными сертификатами

`file_encryption_pass` Пароль для шифрования архива, если поле пустое шифрование отключено. По-умолчанию отключено.
